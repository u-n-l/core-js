...
Geohash.base32 = '0123456789bcdefghjkmnpqrstuvwxyz';
    ...
    Geohash.decode = function(geohash) {
        ...
        return { lat: Number(lat), lon: Number(lon), elevation: Number(geohashWithElevation.elevation), elevationType: geohashWithElevation.elevationType };
    };
    ...
    Geohash.adjacent = function(geohash, direction) {
        ...
        var nextGeohash = parent + Geohash.base32.charAt(neighbour[direction][type].indexOf(lastCh));

        if(elevation && elevationType) return Geohash.appendElevation(nextGeohash, elevation, elevationType);
    ...
    Geohash.excludeElevation = function(geohashWithElevation) {
        ...
        if(geohashWithElevation.includes('#') && geohashWithElevation.includes('@')) throw new Error('Invalid geohash');
        ...
        return {geohash: geohashWithoutElevation, elevation: Number(elevation), elevationType };
    };
     ...
     * @throws  Invalid geohash.
     */
    Geohash.appendElevation = function(geohashWithoutElevation, elevation, elevationType) {
        if(geohashWithoutElevation.length < 0) throw new Error('Invalid geohash');
        if(elevation === 0) return geohashWithoutElevation;
        var elevationChar = '@';
            if(elevationType === 'heightincm')
                elevationChar = '#';
