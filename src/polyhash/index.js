...
function toLocationIds(input, locationIdPrecision = 9) {
  try {
    ...
    if (
      (Array.isArray(input) && input.length && typeof input[0] === "string") ||
      input[0] instanceof String
    ) {
      result = input;
    } else {
...
function compress(locationIds) {
  ...
  const charBitSize = 6;
  const requiredBits = Object.values(polyhash)
    .map((block) => {
      // Block
      return (
        blockHeaderBitSize * block.data.length +
        blockPrecisionHeaderBitSize +
        block.data.reduce((x, y) => {
          // Char
...
function decompress(compressedPolyhash) {
  ...
  const charBitSize = 5;
  for (
    let bufferBitIndex = 0;
    bufferBitIndex < buffer.length * 8 - 1;
    bufferBitIndex++
  ) {
...
function _toTurfPolygon(inputPolygon) {
  ...
  try {
    ...
    if (inputPolygon.features && inputPolygon.features.length) {
      ...
      inputPolygon.geometry.type === "MultiPolygon"
    ) {
      const largestPolygon = inputPolygon.geometry.coordinates.forEach(
        (coords) => {
      ...
      inputPolygon.geometry &&
      inputPolygon.geometry.type === "Polygon"
    ) {
  ...
  polygons.forEach((polygon) => {
    ...
    while (queue.length) {
      ...
      if (intersection) {
        ...
        if (
          _cell.length === locationIdPrecision ||
          (_cell.length <= locationIdPrecision &&
            _isInside(polygon, cellPolygon))
        ) {
          ...
          const intersectionLocationIdsCells = turfMeta
            .coordAll(_newTestPolygon)
            .map((x) => unl.encode(x[1], x[0], locationIdPrecision));
          ...
          const targetCell =
            boundingLocationIdCell.length < _cell.length
              ? _cell
              : boundingLocationIdCell;
...
function inflate(deflatedList) {
  ...
  for (const polyhash of deflatedList) {
    ...
    for (const current of polyhash.data) {
      const locationId = !last
        ? current
        : last.slice(0, precision - current.length) + current;
...
function deflate(locationIds) {
  return locationIds.reduce((result, current, currentIndex, values) => {
    ...
    if (!head) {
      ...
      let i = 0;
      while (
        i < Math.min(head.length, current.length) &&
        head[i] === current[i]
      ) {
        i++;
      }
...
function _isInside(polygon1, polygon2) {
  ...
  return (
    turfBooleanContains(polygon1, polygon2) &&
    !turfBooleanOverlap(polygon1, polygon2)
  );
}
